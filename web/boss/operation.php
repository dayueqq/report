<?php
  ...
  }else if($action=='upcomment'){
	  $reportid=$_GET['reportid'];
	  $comment=$_POST['comment'];
	  $sql="UPDATE report SET comment = '$comment' WHERE report_id='$reportid'";
	  $result=mysql_query($sql);
	  if($result){
		  echo "<script>alert('批注添加成功!');location.href='".$_SERVER["HTTP_REFERER"]."';</script>";
	  }else{
		  echo "<script>alert('批注添加失败!');location.href='".$_SERVER["HTTP_REFERER"]."';</script>";
	  }
  }
  mysql_close();
?>
